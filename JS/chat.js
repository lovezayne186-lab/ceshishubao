...
function enterChat(roleId) {
    ...
    if (chatRoom || historyBox) {
        ...
        const headerHost = ensureChatHeaderAvatar();
        const headerImg = headerHost ? headerHost.querySelector('img') : document.getElementById('current-chat-avatar-img');
        if (headerImg) headerImg.src = profile.avatar || 'assets/chushitouxiang.jpg';
...
async function triggerAI() {
    ...
    for (let i = 0; i < afterLastAi.length; i++) {
        ...
        if (m.type === 'image' && typeof m.content === 'string' && m.content) {
            recentUserImages.push(m.content);
    ...
    if (recentUserImages.length > 0) {
        if (recentUserImages.length === 1) {
            userMessage = { type: 'image', base64: recentUserImages[0], text: lastUserText };
        } else {
            userMessage = { type: 'images', images: recentUserImages, text: lastUserText };
        }
...
function ensureChatHeaderAvatar() {
    const wrap = document.querySelector('.chat-room-title-wrap');
    if (!wrap) return null;
    let host = document.getElementById('current-chat-avatar');
    if (host) return host;
    host = document.createElement('div');
    host.id = 'current-chat-avatar';
    host.style.width = '34px';
    host.style.height = '34px';
    host.style.borderRadius = '8px';
    host.style.overflow = 'hidden';
    host.style.background = '#ddd';
    host.style.marginRight = '10px';
    host.style.flexShrink = '0';
    host.style.boxShadow = '0 1px 4px rgba(0,0,0,0.15)';
    const img = document.createElement('img');
    img.id = 'current-chat-avatar-img';
    img.style.width = '100%';
...
function applyCharacterAvatarToUI(roleId, avatarUrl) {
    ...
    if (window.currentChatRole === roleId) {
        ...
        if (headerImg) {
            ...
            animatedTargets.push(headerImg);
        }
    }
...
async function handleChangeAvatarAction(roleId, payload) {
    ...
    const avatarUrl = normalizeImageUrlCandidate(url);
    if (!avatarUrl || !isValidImageUrlCandidate(avatarUrl)) {
        appendAvatarSwapError(roleId, '我想换，但这张图的链接看起来不太对，加载失败了。');
        return;
    }

    const ok = await preloadImageUrl(avatarUrl, 3500);
    if (!ok) {
        appendAvatarSwapError(roleId, '我想换，但图片加载失败了。');
        return;
    }
...
(function initMemoryArchiveModule() {
    ...
    function summarizeSegment(segmentMsgs) {
        ...
        const majorKeywords = [
            '见面', '约会', '约好', '约定', '表白', '在一起', '分手', '复合', '吵架', '和好', '冷战',
            '旅行', '出差', '搬家', '生病', '住院', '手术', '发烧', '面试', '入职', '离职',
            '考试', '毕业', '生日', '礼物', '纪念日', '同居', '结婚', '订婚'
        ];
        const trivialKeywords = ['吃饭', '吃了', '喝水', '睡觉', '起床', '哈哈', '嗯嗯', '好的', '行', '在吗'];

        const convoText = (Array.isArray(segmentMsgs) ? segmentMsgs : [])
            .filter(m => m && (m.role === 'me' || m.role === 'ai'))
            .map(m => {
                if (m.type === 'image') return (m.role === 'me' ? '用户:' : 'AI:') + '[图片/表情包]';
                const t = normalizeText(m.content);
                if (!t) return '';
                return (m.role === 'me' ? '用户:' : 'AI:') + t;
            })
            .filter(Boolean)
            .join('\n');

        const eventCandidates = [];
        if (containsAny(convoText, majorKeywords)) {
            for (let i = 0; i < userMsgs.length; i++) {
                const m = userMsgs[i];
                const t = normalizeText(m.content);
                if (!t) continue;
                if (!containsAny(t, majorKeywords)) continue;
                if (containsAny(t, trivialKeywords) && !containsAny(t, majorKeywords)) continue;
                eventCandidates.push(t);
            }
        }

        if (eventCandidates.length) {
            const take = eventCandidates.slice(-2);
            take.forEach(t => {
                const short = t.length > 40 ? t.slice(0, 40) + '...' : t;
                events.push(`- 我们：${short}`);
            });
        }
    ...
    window.maybeAutoUpdateMemoryArchive = function (roleId) {
        try {
            ...
            function filterApiEvents(items) {
                const major = ['见面', '约会', '约好', '约定', '表白', '在一起', '分手', '复合', '吵架', '和好', '冷战', '旅行', '出差', '搬家', '生病', '住院', '手术', '发烧', '面试', '入职', '离职', '考试', '毕业', '生日', '礼物', '纪念日', '同居', '结婚', '订婚'];
                const trivial = ['吃饭', '吃了', '喝水', '睡觉', '起床'];
                return (items || [])
                    .map(normalizeSummaryLine)
                    .filter(Boolean)
                    .filter(s => s.length <= 60)
                    .filter(s => /^(我们|今天)/.test(s))
                    .filter(s => containsAny(s, major) || containsAny(segmentText, major))
                    .filter(s => !(containsAny(s, trivial) && !containsAny(s, major)));
            }
