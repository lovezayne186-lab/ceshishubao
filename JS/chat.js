...
function updateInviteCardRow(row, nextStatus) {
    ...
    card.classList.toggle('is-disabled', nextStatus === 'cancelled');
...
function handleInviteClick(action, inviteId) {
    ...
    if (normalized === 'decline') {
        updateInviteStatusInData(roleId, inviteId, 'cancelled');
        if (rowEl) updateInviteCardRow(rowEl, 'cancelled');
        return;
    }
...
function characterAcceptsInvite(inviteId) {
    const roleId = String(window.currentChatRole || '');
    if (!roleId || !inviteId) return;
    ...
    for (let i = 0; i < afterLastAi.length; i++) {
        ...
        if (m.type === 'image' && typeof m.content === 'string' && m.content) {
            ...
            if (c.startsWith('data:image/') || c.startsWith('blob:')) {
                ...
                userTextParts.push(`[用户发送了一个表情包] ${c}`);
            }
...
function appendMessageToDOM(msg) {
    ...
    if (msg.type === 'listen_invite') {
        ...
        const roleProfile = (window.charProfiles && roleId && window.charProfiles[roleId]) ? window.charProfiles[roleId] : {};
        const inviteeName = String(roleProfile.nickName || roleProfile.name || roleId || '').trim();
        const inviteeAvatar = msg.characterAvatar ? String(msg.characterAvatar) : String(roleProfile.avatar || 'assets/chushitouxiang.jpg');
        const status = String(msg.inviteStatus || 'pending');
        const isPending = status === 'pending';
        const isCancelled = status === 'cancelled';
        const isAccepted = status === 'accepted';
        const stateText = isCancelled ? '已取消' : (isAccepted ? '已接受' : '');
        bubbleHtml = `
            <div class="msg-bubble listen-invite-bubble custom-bubble-content">
                <div class="listen-invite-card invite-card-sent${isCancelled ? ' is-disabled is-cancelled' : ''}${isAccepted ? ' is-accepted' : ''}" data-invite-id="${inviteId}">
                    <div class="listen-invite-body">
                        <div class="listen-invite-avatars">
                            ...
                            <div class="listen-invite-avatar">
                                <img src="${inviteeAvatar}" alt="" />
                            </div>
                        </div>
                        <div class="listen-invite-title">${inviteeName || userName}</div>
                        <div class="listen-invite-subtext">邀请你一起听</div>
                        <div class="listen-invite-state">${stateText}</div>
                    </div>
                    ${isPending ? `
                    <div class="listen-invite-actions">
                        <button class="listen-invite-btn listen-invite-btn--decline" type="button" data-action="decline" data-invite-id="${inviteId}" aria-label="拒绝">
                            <i class="bx bx-x" aria-hidden="true"></i>
                        </button>
                        <button class="listen-invite-btn listen-invite-btn--accept" type="button" data-action="accept" data-invite-id="${inviteId}" aria-label="同意">
