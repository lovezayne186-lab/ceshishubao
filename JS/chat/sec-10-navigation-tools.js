...
(function () {
    ...
    function normalizeOfflineReaderSettings(raw) {
        ...
        const backgroundTheme = ['plain', 'paper', 'mist', 'moon', 'night']
        ...
        return { fontSize, colorTheme, backgroundTheme, pageTurn };
    }
 ...
 - 每当你想写抽象判断，请改写成一个可被看见或听见的细节。`; 
    ...
    function buildOfflineCommentPrompt(session, target) {
        ...
        const userName = String(localStorage.getItem('user_name') || '').trim() || '用户';
        ...
        return [
            ...
            '你是一个沉浸式互动小说的单句段评生成器。你只需要围绕一条已经选好的高光句，生成 5 条网友评论。',
            '这些评论不是角色本人说的话，而是围观读者的即时反应。要有网感、现代中文互联网口吻，轻松自然，像在番茄/晋江/微信读书里一边看一边尖叫吐槽。',
            `【重要角色设定】故事主角其实是用户（${userName}），而${roleName}是与用户互动的角色。网友的评论要偏向用户（${userName}），偶尔夹杂着两人互动的评价。严禁将用户（${userName}）和角色（${roleName}）混淆！`,
            wbPrompt,
            '【评论风格要求】为了让段评显得真实且情绪饱满，请扮演不同类型的读者，严格遵守以下 4 种视角来混合生成 5 条评论：',
            '1. 视角偏向用户（娘家人视角）：网友要极其偏爱用户（称呼用户为“我姐”、“女鹅”、“妹妹”等），喜欢看角色被用户“拿捏”的样子。例：“这还说啥了，我姐一句语音把这男人治得服服帖帖！”、“别挣扎了，你小子已经被我女鹅吃得死死的了。”',
            '2. 嗑糖尖叫鸡（CP粉视角）：看到甜的情节要发疯，大量使用感叹号、拟声词。例：“我去，磕到了磕到了啊啊啊啊！”、“嘴角比AK还难压！”、“把民政局给我搬过来！”',
            '3. 沉浸式吐槽/脑洞（显眼包视角）：关注细节，提出搞笑或带入感极强的要求。例：“为什么小说不能播放声音！求小说播放声音教程急急急！”、“十分钟？这男人怕不是要把车开得起飞！”',
            '4. 泥塑/梦女视角：馋角色的身子、被角色的反差萌戳中。例：“救命，这种一边骂骂咧咧一边拿胃药的傲娇爹系男友谁懂啊！”、“他咬后槽牙的样子一定很性感呜呜呜。”',
            '- 评论只能引用正文里真的出现过的原句，不要杜撰没写出来的台词或动作。每条评论尽量短，控制在 10 到 30 个汉字左右，一句话说完。',
            `【当前要评论的句子】\n${String(target && target.quote ? target.quote : '').trim()}`,
            '【输出格式】\n只输出 5 行评论，不要 JSON，不要代码块，不要解释，不要重复原句。\n1. 评论1\n2. 评论2\n3. 评论3\n4. 评论4\n5. 评论5',
            '【数量要求】\n- 固定输出 5 条短评论，一条都不能少。\n- 不要输出 quote，不要编号之外的任何额外前缀。'
        ].filter(Boolean).join('\n\n');
    }
        ...
        try {
            ...
            for (let i = 0; i < targets.length; i++) {
                ...
                const raw = await callNovelApiDirect({
                    systemPrompt: buildOfflineCommentPrompt(session, target),
                    history: [],
                    userText: options.retry
                        ? `上一版评论不够完整。请只围绕这一句重新输出 5 条短评论，不要解释，不要重复原句。\n\n【高光句】\n${target.quote}`
                        : `请只围绕这一句输出 5 条网友评论。\n\n【高光句】\n${target.quote}`,
                    maxTokens: options.retry ? 2048 : 2048
                });
                rawLog.push({ quote: target.quote, raw: raw });
                const lines = parseOfflineCommentLines(raw);
                if (lines.length >= 4) {
                    results.push({
                        quote: target.quote,
                        anchorIndex: target.anchorIndex,
                        comments: lines.slice(0, 5)
                    });
                    continue;
                }
                if (!options.retry) {
                    console.warn('[OfflineNovel Comments] single target insufficient, retrying once', raw);
                    const retryRaw = await callNovelApiDirect({
                        systemPrompt: buildOfflineCommentPrompt(session, target),
                        history: [],
                        userText: `上一版评论被截断了。请重新输出 5 条完整短评论，每条单独一行，不要解释。\n\n【高光句】\n${target.quote}`,
                        maxTokens: 2048
                    });
                    rawLog.push({ quote: target.quote, raw: retryRaw, retry: true });
                    const retryLines = parseOfflineCommentLines(retryRaw);
                    if (retryLines.length >= 4) {
                        results.push({
                            quote: target.quote,
                            anchorIndex: target.anchorIndex,
                            comments: retryLines.slice(0, 5)
                        });
                    }
                }
            }
    ...
    function renderOfflineHistory(session) {
        ...
        msgs.forEach(m => {
            ...
            if (m.type === 'long_text' && m.payload && Array.isArray(m.payload.content)) {
                ...
                const pendingAnchorIndexes = Array.isArray(m.payload.pendingAnchorIndexes) ? m.payload.pendingAnchorIndexes : [];
                const commentTargets = Array.isArray(m.payload.commentTargets) ? m.payload.commentTargets : [];
                ...
                m.payload.content.forEach(p => {
                    ...
                    if (related.length || shouldShowPending || shouldShowError || shouldShowTarget) {
                        ...
                        trigger.type = 'button';
                        trigger.style.cssText = 'position:relative; z-index:2; margin-top:8px; width:100%; background:transparent; border:none; padding:0; display:flex; align-items:center; gap:10px; cursor:pointer; opacity:0.85; transition:opacity 0.2s;';
                        trigger.onmouseenter = function() { trigger.style.opacity = '1'; };
                        trigger.onmouseleave = function() { trigger.style.opacity = '0.85'; };

                        const line = document.createElement('div');
                        line.style.cssText = 'flex:1 1 auto; border-bottom:1px dashed rgba(139,94,52,0.25); height:1px;';
                        ...
                        badge.style.cssText = 'flex:0 0 auto; padding:4px 8px; border-radius:6px; background:rgba(255,255,255,0.6); color:#8b5e34; border:1px solid rgba(139,94,52,0.15); font-size:11px; font-weight:600; box-shadow:0 2px 8px rgba(139,94,52,0.05); backdrop-filter:blur(4px);';
