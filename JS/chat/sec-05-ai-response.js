...
function handleActions(roleId, actions) {
    const result = { postMessages: [], offlineMeetingStart: false, offlineMeetingSource: '' };
    ...
    try {
        ...
        if (offlineMeetingAct && typeof offlineMeetingAct === 'object' && offlineMeetingAct.start) {
            ...
            result.offlineMeetingSource = String(offlineMeetingAct.source || '').trim();
        }
    } catch (e) {
    ...
    if (Array.isArray(replyValue)) {
        for (let i = 0; i < replyValue.length; i++) {
            ...
            if (rawType === 'offline_action' || rawType === 'action' || rawType === 'narration') {
                pushObject('offline_action', { content: String(item.content != null ? item.content : item.text || '').trim() });
                continue;
            }
    ...
    if (typeof window.callAI === 'function') {
        window.callAI(
            ...
            async (aiResponseText) => {
                ...
                const familyParsed = parseAIFamilyCardFromContent(joinedReplyTextForQuickCheck || '');
                const familyCardAmountFromText = familyParsed.amount;
                rawText = familyParsed.text || '';
                if (!jsonPayload || !Array.isArray(jsonPayload.reply)) {
                    replySegments = normalizeStructuredReplySegments(rawText, {
                        offlineMode: wasOfflineMeeting || !!(actions && actions.offlineMeetingStart)
                    });
                }
                ...
                for (let i = 0; i < parts.length; i++) {
                    ...
                    if (segment.kind === 'offline_action') {
                        ...
                        if (actionText) {
                            ...
                            ensureChatMessageId(actionMsg);
                            tryAttachQuote(actionMsg);
                            window.chatData[roleId].push(actionMsg);
                    ...
                    if (segment.kind === 'voice' || segment.kind === 'photo' || segment.kind === 'location' || segment.kind === 'sticker') {
                        ...
                        if (directContent || directMsgType === 'location') {
                            ...
                            ensureChatMessageId(directMsg);
                            tryAttachQuote(directMsg);
                            window.chatData[roleId].push(directMsg);
                            appendMessageToDOM(directMsg);
                        }
                        if (i < parts.length - 1) {
