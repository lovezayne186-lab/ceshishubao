...
- 当前时间：${timeStr}（24小时制）
- 时间段：${dayPeriodCn}
- 中文时间描述：${timeDescCn}（例如：23:00=晚上11点，11:00=上午11点）
- 公历日期：${solarDate}
- 农历日期：${lunarDate}
...
4. URL：一旦你决定更换，必须返回 actions.changeAvatar = { "url": "IMAGE_URL" }，其中 IMAGE_URL 必须严格等于用户提供的图片 URL（严禁编造）。
...
`;
    ...
    if (imageUrls.length > 0) {
        ...
        messages.push({
            role: "system",
            content:
                `[System: User sent ${imageCount} image(s). If the user explicitly asks you to change your avatar, decide whether to accept based on your persona and status.favorability. If you accept, you MUST set actions.changeAvatar = { "url": "IMAGE_URL" } using EXACTLY ONE of the image_url.url values provided by the user. If the user does not clearly ask to change avatar, keep actions.changeAvatar = null and do not change it.]`
        });
    }
...
1. 记录我从这次聊天里真正记住、在意的内容：
   ...
   - 我和对方一起经历的值得记住的重大事件或关键时刻（对未来关系/计划/情绪有影响的；日常吃饭闲聊不算）；
   - 我当下真实的心情和小小的想法。
2. 不要写“我们聊了很多”“对话很愉快”这种空话，要有具体细节。
3. 不要流水账，只挑对未来相处有帮助、我真的会记在心里的信息。
4. 严禁推测与脑补：likes/habits/events 里每一条都必须能在【用户:】消息中找到清晰的字面依据；找不到就不要写。
...
- events：只写重大事件或关键进展；每条必须以“我们...”或“今天...”开头，且能在对话中找到依据。
