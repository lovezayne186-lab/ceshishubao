...
function summarizeDevAiUserMessage(userMessage) {
    try {
        if (userMessage && typeof userMessage === 'object') {
            if (typeof userMessage.text === 'string' && userMessage.text.trim()) {
                return userMessage.text.trim().slice(0, 120);
            }
            if (userMessage.type === 'image') return '[图片请求]';
            if (userMessage.type === 'images') return '[多图请求]';
            return '[结构化请求]';
        }
...
if (isModernPromptClass) {
    ...
    if (modernPromptMeta && modernPromptMeta.promptClass === 'full_chat') {
        privateTransportGuard.push('- thought 只做内部推理；reply 只放给用户看的最终内容；system_event 仅在 reply 为 null 时使用。');
...
try {
    ...
    function buildApiMessages(finalSystemPrompt, historyMessages, userMessage) {
        ...
        const disableTimePerceptionForThisCall = !!isOfflineMeetingV2;
        ...
        if (imageUrls.length > 0) {
            const blocks = [];
            // 使用 currentNow 传递
            const wrapper = disableTimePerceptionForThisCall
    ...
    try {
        ...
        if (msg.indexOf('Failed to fetch') !== -1) {
            onError("连接断开了：" + msg + "（可能是 SSL/TLS 或网络拦截导致）");
            return;
        }
        onError("连接断开了：" + msg);
    }
